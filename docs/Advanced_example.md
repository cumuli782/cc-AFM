This example demonstrates the calculation of a series of cc-AFM images of the 2-Iodotriphenylene on Ag(111) from the full electronic density. The calculations include the effects of the probe-particle deflection and the probe-tip interaction. Starting point are the charge density of the sample (*CHG_sample*), the charge density of the probe-particle (*CHG_tip*) and the electrostatic potential (*LOCPOT*) as they are calculated by VASP (all can be found in the *[example](https://github.com/cumuli782/cc-AFM/edit/main/example)* folder). The resulting images should be show some similarity to figure 5 in ... (they will differ quite significantly, since the resolution of all files has been reduced for the upload. This effects each step of the calculation and the errors add up)

**Note:** ppafm is parallelized with OpenMp and extract_isosurface_height_map_parallel.py is parallelized with the multiprocess package. Since both can utilize several cores, it is best to run them on a multicore machine. 
...
## 1a) params.ini
A full parameter file is present in the example directory. While nothing has to be changed here, some parameters should be disussed. As discussed in the basic workflow, most of the parameters are set as in a regular constant height AFM calculation. In this case, the setup is chosen to emulate a CO molecule adsorbed on a silver tip. The parameters which have to be chosen specially for the cc-AFM calculation are the scan range (*ScanMin* and *ScanMax*) and the scan resolution (*ScanStep*). Since the unit cell of the system is rectangular, the scan range was chosen to be as large as the unit cell in x and y direction. While the z direction can also ecompass the whole cell, the scan range has been reduced, since the values above and below the thresholds will not contribute to the cc-AFM images. This saves on input and output operations. The resolution has been increased in z-direction to accomodate for the sensetivity of the cc-AFM method in this direction. Furthermore the resolution was decreased in x and y direction to fit the reduced resolution in the charge densities. (For your actual calculations ensure that your charge density grid has at least a resolution of 0.1 Angstrom)
...
```
The first three arguments define the charge density file, the prefix for the output files, and the isovalue. However, the isovalue is overwritten by options later on.
...
Without further options, the program creates the png images for the cc-AFM image (as grayscale) and the cc-STM image (as gnuplot colorscale). By specifying *-aat* (AFM as text) the frequency shifts are additionally stored as a txt file, where the first two columns represent the x,y position of the gridpoint, and the third column represents the frequency shift.

# Remarks
## STM image
The constant current STM images generated by Afm_at_Isosurface.py are generated directly from the isosurface files, they span the whole unit cell. To confine the STM images to the AFM grid, specify the option *-peh* (output as .png) or *-sat* (output as .txt).

## Partial densities
It is also possible to generate cc-AFM images using a partial electronic density which corresponds more closely to the density measured in STM experiments. The procedure stays the same, simply exchange the total density in step 2 (but not in step 1!) by the partial density.
