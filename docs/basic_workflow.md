...
1) The scan area (defined by ScanMin and ScanMax) has to be smaller or equal to the cell size of CHG.xsf -- the scan area has to fit in the charge density cell. To be more preceise, each datapoint on the AFM grid has to be encompassed by at least one further outward lying density grid datapoint. This means, the CHG cell has to be big enough, that the area of interest (the molecule and its surroundings) can be covered by the rectangular cell defined by scanMin and scanMax. The z coordinate of scanMin and scanMax can be chosen as large as the CHG cell, but can be chosen smaller in order to save on output data.
2) The cc-AFM images are sensitive to changes in z direction. Therefore a finer grid is recommended in z direction. Change ScanStep accordingly, for example ScanStep 0.1 0.1 0.01. The ScanStep in x and y direction should also be larger than the grid distance of the charge density grid to ensure, that the charge density grid encompasses the AFM grid
